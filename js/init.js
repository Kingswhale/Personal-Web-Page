...
function devman_tm_modalbox() {
  'use strict'
  jQuery('.devman_tm_all_wrap').prepend(
    '<div class="devman_tm_modalbox"><div class="box_inner"><div class="close"><a href="#"><i class="icon-cancel"></i></a></div><div class="description_wrap"></div></div></div>'
  )
}
...
function devman_tm_service_popup() {
  ...
  var button = jQuery(
    '.devman_tm_service .service_list ul li .devman_tm_full_link'
  )
...
function devman_tm_modalbox_news() {
  ...
  var modalBox = jQuery('.devman_tm_modalbox')
  var button = jQuery(
    '.devman_tm_news .devman_tm_full_link,.devman_tm_news .details .title a'
  )
  var closePopup = modalBox.find('.close')
  button.on('click', function () {
    var element = jQuery(this)
...
function devman_tm_accordion() {
  ...
  acc.each(function () {
    ...
    element
      .find('.accordion_in:nth-child(' + active + ')')
      .addClass('acc_active')
    element
      .find('.accordion_in:nth-child(' + active + ') .acc_content')
      .slideDown({ duration: s, easing: a })
  })
  ...
  head.off().on('click', function () {
    ...
    if (!c.hasClass('acc_active')) {
      if (t === 'accordion') {
        p.find('.acc_active')
          .removeClass('acc_active')
          .find('.acc_content')
          .slideUp({ duration: s, easing: a })
      }
...
function sendMail() {
  ...
  const serviceID = 'service_bgpg7gy'
  const templateID = 'template_psrxkh9'
